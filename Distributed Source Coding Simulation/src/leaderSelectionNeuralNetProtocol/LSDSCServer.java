...
public class LSDSCServer implements stimulator.Server{
	...
	public boolean receiveData(String data, int id) {
		...
		//System.out.println("Received from : "+id+" Leader is "+leader+" requested_bits: "+requested_bits[id]+" received data length: "+data.length());
		if(id!=leader &&round_number>=K && data.length() != requested_bits[id]+1 && data.length() !=0) return false;
		
		//if(!is_first_round[id] && id!=leader && ! obtained_sensor_reading[leader]) return false;
		
		//leader_reading / leader everybody is un-coded, and is assumed to be correct
